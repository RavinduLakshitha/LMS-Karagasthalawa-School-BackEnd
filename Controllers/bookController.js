...
exports.updateBook = async (req, res) => {
  ...
  try {
    ...
    if (!updatedBook) {
      return res
        .status(404)
        .json({ message: "Book not found with the given ID: ${id}" });
    }
...
exports.getBookCountByName = async (req, res) => {
  try {
    const bookCounts = await Book.aggregate([
      ...
      {
        $project: { bookName: "$_id", count: 1, _id: 0 }, // Format output
      },
    ]);

    res.status(200).json(bookCounts);
  } catch (error) {
